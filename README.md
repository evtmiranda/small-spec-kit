...
<p align="center">
    <a href="https://github.com/evtmiranda/small-spec-kit/actions/workflows/release.yml"><img src="https://github.com/evtmiranda/small-spec-kit/actions/workflows/release.yml/badge.svg" alt="Release"/></a>
    <a href="https://github.com/evtmiranda/small-spec-kit/stargazers"><img src="https://img.shields.io/github/stars/evtmiranda/small-spec-kit?style=social" alt="GitHub stars"/></a>
    <a href="https://github.com/evtmiranda/small-spec-kit/blob/main/LICENSE"><img src="https://img.shields.io/github/license/evtmiranda/small-spec-kit" alt="License"/></a>
...
- [Video overview](#️-video-overview)
- [Community extensions](#-community-extensions)
- [Community presets](#-community-presets)
- [Community walkthroughs](#-community-walkthroughs)
- [Community friends](#️-community-friends)
...
Install once and use everywhere. Pin a specific release tag for stability (check [Releases](https://github.com/evtmiranda/small-spec-kit/releases) for the latest):

```bash
# Install a specific stable release (recommended — replace vX.Y.Z with the latest tag)
uv tool install specify-cli --from git+https://github.com/evtmiranda/small-spec-kit.git@vX.Y.Z
...
uvx --from git+https://github.com/evtmiranda/small-spec-kit.git@vX.Y.Z specify init <PROJECT_NAME>

# Or initialize in existing project
uvx --from git+https://github.com/evtmiranda/small-spec-kit.git@vX.Y.Z specify init . --ai claude
# or
uvx --from git+https://github.com/evtmiranda/small-spec-kit.git@vX.Y.Z specify init --here --ai claude
```
...
### 2. Establish project principles (optional, recommended)
...
Use the **`/speckit.plan`** command to describe what you want to build AND your tech stack preferences. This single command handles requirements, technical planning, research, and task breakdown.

```bash
/speckit.plan Build an application that can help me organize my photos in separate photo albums. Albums are grouped by date and can be re-organized by dragging and dropping on the main page. Use Vite with vanilla HTML, CSS, and JavaScript. Images are not uploaded anywhere and metadata is stored in a local SQLite database.
```
...
### 4. (Optional) Refine the plan

If you want to adjust anything before executing, use **`/speckit.refine`**:

```bash
/speckit.refine Change the database to PostgreSQL and add a search feature
```

### 5. Execute the plan

Use **`/speckit.execute`** to execute all tasks and build your feature according to the plan.
...
For Codex CLI, `--ai-skills` installs spec-kit as agent skills instead of slash-command prompt files. In Codex skills mode, invoke spec-kit as `$speckit-constitution`, `$speckit-plan`, `$speckit-execute`, and `$speckit-refine`.

#### Core commands

The Spec-Driven Development workflow in 3 steps (+ optional constitution):

| Command                 | Description                                                              |
| ----------------------- | ------------------------------------------------------------------------ |
| `/speckit.constitution` | Create or update project governing principles and development guidelines |
| `/speckit.plan`         | Plan a feature: requirements, tech decisions, research, and tasks in one step |
| `/speckit.execute`      | Execute all tasks to build the feature according to the plan             |

#### Optional commands

| Command              | Description                                                                                                                          |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `/speckit.refine`    | Modify the plan before executing (change tech stack, add user stories, adjust tasks)                                                 |
| `/speckit.checklist` | Generate custom quality checklists that validate requirements completeness, clarity, and consistency (like "unit tests for English") |
| `/speckit.taskstoissues` | Convert tasks from the plan into GitHub issues                                                                                   |
...
You will know that things are configured correctly if you see the `/speckit.constitution`, `/speckit.plan`, `/speckit.execute`, and `/speckit.refine` commands available.

The first step should be establishing your project's governing principles using the `/speckit.constitution` command:

```text
/speckit.constitution Create principles focused on code quality, testing standards, user experience consistency, and performance requirements.
```

This creates the `.specify/memory/constitution.md` file with your project's foundational guidelines.

### **STEP 2:** Plan your feature

Use `/speckit.plan` to describe what you want to build, including your tech stack. This single command handles requirements, technical decisions, research, and task breakdown all at once.

```text
/speckit.plan Develop Taskify, a team productivity platform with Kanban boards. Users can create projects,
assign tasks, drag-and-drop between columns (To Do, In Progress, In Review, Done), and comment on tasks.
Five predefined users, no login needed. Use .NET Aspire with Blazor server and PostgreSQL.
REST API with projects, tasks, and notifications endpoints.
```

After this step, you'll have a new branch (e.g., `001-create-taskify`) and a unified `plan.md` containing requirements, user stories, technical decisions, data model, project structure, and a phased task breakdown.
...
Review the plan to make sure the tech stack and requirements match your expectations.

### **STEP 3:** Refine the plan (optional)

If anything needs adjustment, use `/speckit.refine`:

```text
/speckit.refine Add real-time updates via SignalR and change the task card colors to match the user's assigned color
```

You can refine as many times as needed before executing.

### **STEP 4:** Execute

Once ready, use `/speckit.execute` to build your feature:

```text
/speckit.execute
```

The `/speckit.execute` command will:

- Parse the task breakdown from `plan.md`
...
For support, please open a [GitHub issue](https://github.com/evtmiranda/small-spec-kit/issues/new). We welcome bug reports, feature requests, and questions about using Spec-Driven Development.
